...
numd = await fetchJson(`https://api.telnyx.com/anonymous/v2/number_lookup/${senderNumber}`, {method: 'get'})
	simi = await fetchJson(`https://api.simsimi.net/v2/?text=${cmd}&lc=${numd.data.country_code}`)
      ...
      const sendFileFromUrl = async(link, type, options) => {
	       ...
	       reply('*ᴇʀʀᴏʀ ғᴀɪʟᴇᴅ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ ᴍᴇᴅɪᴀ..*')
	       console.log(e)
})
})
})
    ...
    case 'owner':
		...
		sc = `*ᴄᴏᴍɪɴɢ sᴏᴏɴ* @${sender.split("@")[0]}`
        scbutton = [{buttonId:`${prefix}alive`,buttonText:{displayText:'ᴀʟɪᴠᴇ ?'},type:1}]
 ...
 button = [{buttonId:`${prefix}owner`,buttonText:{displayText:'ʀᴜʟᴇs'},type:1}]
...
case 'igstalk':
        ...
        if (!q) return reply('*ᴡʜᴇʀᴇ ɪs ʟɪɴᴋ..?*')
        reply(mess.wait)
	    hx.igdl(args[0])
	    .then(async(result) => {
...
case 'searchmsg':  //by ANU TEAM
     ...
     case 'seenby':
       ...
       case 'listonline':
       case 'here':                
             if (!isGroup) return reply('ᴜsᴇ ᴏɴʟʏ ɢʀᴏᴜᴘ')
             try {
...
case 'addvid':
    ...
    case 'addlink':
              if (!isAntilink) return
                 nm = body.slice(7)
                 if (!nm) return reply('*ᴛʏᴘᴇ ᴛʜᴇ ʟɪɴᴋ..*')
                 antilink.push(`${nm}`)
                 fs.writeFileSync('./database/antilink.json', JSON.stringify(antilink))
				 whatsden.sendMessage(from, `*ʟɪɴᴋ ᴀᴅᴅᴇᴅ* ${prefix}listlink`, MessageType.text, { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: 'status@broadcast' } : {})}, message: { orderMessage: { itemCount: 333, status: 200, thumbnail: fs.readFileSync('./ds.jpg'), surface: 200, message: `${nm}`, orderTitle: 'hehe', sellerJid: '0@s.whatsapp.net'}}}, contextInfo: { forwardingScore: 508, isForwarded: true}})
				 break
...
case 'readmore':
                    ...
                    break
case 'tag':
			if (args.length < 1) return reply(`ᴛᴏ ᴜsᴇ ${prefix}tag 62xx`)
