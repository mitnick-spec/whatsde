...
case 'emoji':
   ...
   case 'takestik':
                    if(body == "") return reply(`Kirim perintah *${prefix}${command} [ pack|author ]*\nContoh : ${prefix}${command} punya|elios`)
                    reply(mess.wait)
                    p = body
                    text = p.split('|')
                    const stki = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo
                    const stiku = await whatsden.downloadAndSaveMediaMessage(stki)
                    whatsden.sendMessage(from, stiku.toString('base64'), sticker, {pack: `${text[0]}`, author: `${text[1]}`, emojis: body.match(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g)})
                    break
    case 'photo':
    case 'toimg':
...
if (mek.message.extendedTextMessage === null || mek.message.extendedTextMessage === undefined) return;
if (mek.message.extendedTextMessage.contextInfo.participant === undefined) {
entah = mek.message.extendedTextMessage.contextInfo.mentionedJid[0]
if (entah.length > 1) {
var mems_ids = []
for (let ids of entah) {
mems_ids.push(ids)
